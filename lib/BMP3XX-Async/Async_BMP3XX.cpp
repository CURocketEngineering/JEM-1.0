...
/**************************************************************************/
/*!
    @brief (NEW) Begins a sensor conversion in forced mode without blocking.
    
    This function writes the desired settings and sets the sensor into forced mode.
    It immediately returns and records the start time of the conversion.
    
    You must later call updateConversion() to retrieve the data once the sensor has finished its conversion.
*/
/**************************************************************************/
void Adafruit_BMP3XX::startConversion(void) {
  uint16_t settings_sel = 0;
  uint8_t sensor_comp = 0;
  ...
  // Enable filter and ODR if desired
  if (_filterEnabled) {
    settings_sel |= BMP3_SEL_IIR_FILTER;
  }
  if (_ODREnabled) {
  ...
  if (rslt != BMP3_OK) {
#ifdef BMP3XX_DEBUG
    Serial.print("Error setting sensor settings: ");
    Serial.println(rslt);
#endif
    return;
  }
  
  // Set sensor into forced mode to begin conversion.
  the_sensor.settings.op_mode = BMP3_MODE_FORCED;
  rslt = bmp3_set_op_mode(&the_sensor);
  if (rslt != BMP3_OK) {
#ifdef BMP3XX_DEBUG
    Serial.print("Error setting op mode: ");
    Serial.println(rslt);
#endif
    return;
  }
  
  // Record the start time (non-blocking: we do not wait here)
  _meas_start = millis();
}

/**************************************************************************/
/*!
    @brief (NEW) Checks if the sensor conversion is complete and, if so, reads the data.
    
    This function should be called repeatedly from the main loop. It compares the elapsed time
    since startConversion() with the expected conversion delay (_conversionDelay). If the delay has passed,
    it calls bmp3_get_sensor_data() to fetch the sensor data.
    
    @return True if the conversion is complete and data is updated, False if not ready yet.
*/
/**************************************************************************/
bool Adafruit_BMP3XX::updateConversion(void) {
  // Check if enough time has passed (non-blocking)
  if ((millis() - _meas_start) < _conversionDelay) {
    return false;  // Conversion not yet complete.
  }
  
  // Enough time has elapsed; now read the sensor data.
  uint8_t sensor_comp = BMP3_TEMP | BMP3_PRESS;
  struct bmp3_data data;
  int8_t rslt = bmp3_get_sensor_data(sensor_comp, &data, &the_sensor);
  if (rslt != BMP3_OK) {
#ifdef BMP3XX_DEBUG
    Serial.print("Error getting sensor data: ");
    Serial.println(rslt);
#endif
    return false;
  }
