...
public:
  ...
  bool updateConversion(void); // returns true when new data is available
  float getTemperature(void) { return temperature; }
  float getPressure(void) { return pressure; }
  void setConversionDelay(uint32_t delay) { _conversionDelay = delay; }
  uint32_t getConversionDelay(void) { return _conversionDelay; }
...
private:
  ...
  uint32_t _meas_start;      // time when conversion was started (millis)
  uint32_t _conversionDelay; // expected conversion delay in ms
